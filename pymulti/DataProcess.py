        ...
        for i in range(len(float_matrix)):
            for j in range(len(float_matrix[0])):
                if np.isnan(float_matrix[i][j]):
                    ...
                    # 将浮点数格式化为字符串，并写入文件
                    file.write("%.6f " % float_matrix[i][j])
            file.write("\n")
...
def get_max_along_r_shock_wave_area(interp_list, LR_r_list):
...
def get_integrated_along_r_shock_wave_area(value_interp_list, rho_interp_list, LR_r_list):
...
# 取每条轴上check_interp_list∈check_range_pair的区域的value积分值
def get_integrated_along_r_all(value_interp_list, r_list, check_interp_list, check_range_pair):
    int_val_list = []  # 存积分值
    for i in range(len(value_interp_list)):
        int_val = 0
        # 这里为了不数组越界，不考虑第一个值（事实上第一个值一般无效）
        for j in list(range(len(value_interp_list[i])))[1:]:
...
def filename_time_list(filenames):  # 将一个filenames字符串列表中的数字相关的字符串提取出，按数字大小排序
    ...
    file_time_list = [[filename, float(filename)]
...
def function_time_list(case: io.Cases, time_range_pair, func):
    ...
    for file_time_pair in sorted_file_time_list:
        if time_range_pair[0] <= file_time_pair[1] and file_time_pair[1] <= time_range_pair[1]:
            function_time_pair_list.append(
                [func(case, file_time_pair[0]), file_time_pair[1]])

    return function_time_pair_list


# 将一个数据的时间列表平滑化
def smooth_val_time_list(val_time_pair_list, interval=1e-11, kernel_array=[1/50, 1/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 2/25, 1/25, 1/50]):
    '''
    ...
    new_time_list = np.linspace(
        time_list[0], time_list[-1], int(np.round((time_list[-1]-time_list[0])/interval+1)))
...
def judge_func_depo(case: io.Cases):  # 为depo特制的Judge_func
    ...
    def single_file_judge(case: io.Cases, filename: str):
        ...
        int_depo_list = get_integrated_along_r_all(
            depo_interp_list, r_list, depo_interp_list, [1e18, 1e30])
        return np.sqrt(np.var(int_depo_list))/np.mean(int_depo_list)

    val_time_pair_list = function_time_list(
        case, [0.89e-9, 1.11e-9], single_file_judge)
